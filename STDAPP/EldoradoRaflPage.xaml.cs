...
{
    /// <summary>
    /// Логика взаимодействия для EldoradoRaflPage.xaml
    /// </summary>
    public partial class EldoradoRaflPage : Page
    {
        public EldoradoRaflPage()
        {
            InitializeComponent();
            DpiFactorContainer.Resize(EldoradoGreed);
        ...
        public DataEdit SaveMenuData()  // функция для сохранения всех полей при создании таска
        {
            ...
            DataNow.PsVersion = PsVersion.SelectedIndex;
            return DataNow;
        }
        ...
        private void AddTask_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < TasksCounter; i++)  // цикл по созданию таков от введенного числа 
                {
                    ...
                    EldoradoTaskClass eldoradoTask = new EldoradoTaskClass();  // создаем экземпляр основного таска                                                        //


                    if (Accounts.SelectedItem != null)  // Добавление аккаунта
                    {
                        if (AccountsChoose.SelectedIndex == 0)
                        {
                            int AccIndex = AccountData.GiveAccountIndexEditor(Accounts.SelectedItem.ToString());
                            ...
                            {
                                AccIndex = AccountData.GiveAccountIndexEditor(Accounts.SelectedItem.ToString());
                                DataToTask.Account = AccountData.GiveMeAccount(AccIndex, Accounts.SelectedItem.ToString());
                            }
                        ...
                        else
                        {
                            ...
                            string[] ShowAcc = AccountsChoose.SelectedItem.ToString().Split('@');
                            mytask.Account.Content = ShowAcc[0];
                        } // ======
                        ...
                        DataToTask.PsVersion = PsVersion.SelectedIndex;
                        ...
                        mytask.ButtonStart.Click += (x, g) => // добавление действия кнопки старт
                        {
                            ...
                            try
                            {
                                TaskList[CreatedTasksForButton - NumberOfDeleted] = new Task(() => eldoradoTask.src(CreatedTasksForButton, mytask, DataToTask, token));

                                TaskList[CreatedTasksForButton - NumberOfDeleted].Start();
                            }
                            catch
                            {
                                TaskList[CreatedTasksForButton] = new Task(() => eldoradoTask.src(CreatedTasksForButton, mytask, DataToTask, token));
        ...
        private void AutoregOpenButton_Click_1(object sender, RoutedEventArgs e)
        {
