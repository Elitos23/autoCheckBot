...
{
    /// <summary>
    /// Логика взаимодействия для ProfileEditorPage.xaml
    /// </summary>
    public partial class ProfileEditorPage : Page
    {
        public ProfileEditorPage()
        {
            ...
            DpiFactorContainer.Resize(EditorGreed);
        }
        ...
        private void AddTask_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < TasksCounter; i++)  // цикл по созданию таков от введенного числа 
                {
                    ...
                    ProfileEditorTaskClass TSK = new ProfileEditorTaskClass(); // создаем экземпляр основного таска
                                                                               //
                    ...
                    if (Accounts.SelectedItem != null)  // Добавление аккаунта
                    {
                        ...
                        mytask.ButtonStart.Click += (x, g) => // добавление действия кнопки старт
                        {
                            ...
                            try
                            {
                                TaskList[CreatedTasksForButton - NumberOfDeleted] = new Task(() => TSK.Src(CreatedTasksForButton, DataToTask, mytask));

                                TaskList[CreatedTasksForButton - NumberOfDeleted].Start();
                            }
                            catch
                            {
                                TaskList[CreatedTasksForButton] = new Task(() => TSK.Src(CreatedTasksForButton, DataToTask, mytask));
                        ...
                        mytask.ButtonPause.Click += (x, g) =>
                        {
                            ...
                            if (ProfileEditorTaskClass.browserArEditor[CreatedTasksForButton] != null)
                            {

                                ProfileEditorTaskClass.browserArEditor[CreatedTasksForButton].CloseAsync();
